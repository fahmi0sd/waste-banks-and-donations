...
func NewController(logger *slog.Logger, service category.Service) *Controller {
...
func (ctrl *Controller) Create(c echo.Context) error {
	...
	if err := ctrl.validate.Struct(req); err != nil {
		return c.JSON(http.StatusBadRequest, response.Error("name wajib diisi, unit hanya boleh kg/liter"))
	}
	...
	return c.JSON(http.StatusCreated, response.Success("kategori sampah berhasil dibuat", created))
}
...
func (ctrl *Controller) List(c echo.Context) error {
	...
	return c.JSON(http.StatusOK, response.Success("daftar kategori sampah", list))
}
...
func (ctrl *Controller) Get(c echo.Context) error {
	...
	cat, err := ctrl.service.Get(id)
	if err != nil {
		return c.JSON(http.StatusNotFound, response.Error(err.Error()))
	}
	return c.JSON(http.StatusOK, response.Success("detail kategori sampah", cat))
}
...
func (ctrl *Controller) Update(c echo.Context) error {
	...
	return c.JSON(http.StatusOK, response.Success("kategori sampah berhasil diperbarui", updated))
}
...
func (ctrl *Controller) Delete(c echo.Context) error {
	...
	return c.JSON(http.StatusOK, response.Success("kategori sampah berhasil dihapus", nil))
}
